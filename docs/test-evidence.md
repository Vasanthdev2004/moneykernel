...
| manual SHADOW, supported agent session (prd.md 16.5 route): `pnpm agent:run -- --context-out`, proposal authored by this Claude Code session, `pnpm agent:run -- --provider agent-session --proposal ... --context-in ...` | dump → submit in 1.3 s (inside the 5 s observation freshness window); kernel 201 `COUNTERPROPOSE` 0.145 SOL @ 102.8 (`SIZE_NORMALIZED`, mark 102.68); operator approval; dispatch; paper fill 0.145 SOL @ 102.69 for 14.89005 USDT, fee 0.01489005; `reconciled_at` set. Trace, context dump, proposal file and kernel outcome are committed under `docs/evidence/model-runs/` (no tokens). Provenance label `SUPPORTED_AGENT_SESSION` lives in the runner trace; receipts keep `SCRIPTED` for this seeded agent per decision 0003 until G6 binds labels to run evidence | one real model proposal through the agent API with strict validation |
